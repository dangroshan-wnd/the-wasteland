...
from fantasy_football_ingestion.scrapers.pro_football_reference import (
    scrape_inprogress_season_schedule,
)
...
# One HTML scrape per in-progress season. PFR uses visitor/home columns until
# the season is complete; the completed-season scraper stays on Winner/tie.
# Full replace per season on each run.
...
def scrape_season_games(season):
    try:
        df = scrape_inprogress_season_schedule(season)
        if df.empty:
            return []
        ...
        print(f"⚠️ Failed to scrape in-progress schedule for season {season}: {e}")
        return None
...
def replace_season_schedules(conn, season, games):
    with conn.cursor() as cur:
        cur.execute(
            """
            DELETE FROM landing.pfr_inprogress_season_schedules
        ...
        for payload, game_id in games:
            cur.execute(
                """
                INSERT INTO landing.pfr_inprogress_season_schedules (payload, game_id)
                VALUES (%s, %s)
                """,
...
def main():
    ...
    total_inserted = 0
    for i, season in enumerate(seasons):
        print(f"\n🔄 Scraping in-progress schedule for {season}...")
        games = scrape_season_games(season)
        if games is None:
