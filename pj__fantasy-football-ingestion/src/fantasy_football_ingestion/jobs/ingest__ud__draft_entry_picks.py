...
# One API call per draft (GET /v2/drafts/{id}); picks stored as raw API objects in JSONB.
# draft_id is kept as a table column (not in payload) for incremental skip logic.
...
def get_existing_draft_ids(conn):
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT DISTINCT draft_id FROM landing.ud_draft_entry_picks")
            return {r[0] for r in cur.fetchall() if r[0] is not None}
    except Exception as e:
        print(f"⚠️ Couldn't query landing.ud_draft_entry_picks, proceeding with full scrape: {e}")
        return set()
...
def extract_picks_from_draft(raw):
    return raw.get("draft", {}).get("picks", [])


def collect_draft_entry_picks(token, draft_ids, conn=None, *, record_limit=None):
    ids_to_run = draft_ids[:record_limit] if record_limit else draft_ids
    all_picks = []
    inserted = 0

    try:
        for draft_id in tqdm(ids_to_run, desc="Fetching picks", unit="draft"):
            raw = fetch_full_draft(draft_id, token)
            if raw == "unauthorized":
                return all_picks, inserted, True
            if raw:
                picks = [
                    (p, draft_id) for p in extract_picks_from_draft(raw) if isinstance(p, dict)
                ]
                if conn is not None:
                    inserted += insert_draft_entry_picks(conn, picks)
                else:
        ...
        return all_picks, inserted, False

    return all_picks, inserted, False


def insert_draft_entry_picks(conn, picks):
    if not picks:
        return 0

    inserted = 0
    with conn.cursor() as cur:
        for pick, draft_id in picks:
            cur.execute(
                """
                INSERT INTO landing.ud_draft_entry_picks (payload, draft_id)
                VALUES (%s, %s)
...
def write_sample_json(picks):
    SAMPLE_JSON_PATH.parent.mkdir(parents=True, exist_ok=True)
    payloads = [pick for pick, _draft_id in picks]
    with SAMPLE_JSON_PATH.open("w") as f:
        json.dump(payloads, f, indent=2)
    print(f"📄 Wrote {len(payloads)} draft entry picks to {SAMPLE_JSON_PATH}")
...
def main():
    ...
    if TEST_MODE:
        picks, _inserted, unauthorized = collect_draft_entry_picks(
            token, draft_ids, record_limit=TEST_MODE_RECORD_LIMIT
        )
        if picks:
            write_sample_json(picks)
        if unauthorized:
            print(
                f"🛑 Stopping pull due to expired token. {len(picks)} picks saved to sample file."
            )
            return
        print(f"\n✅ Done. {len(picks)} draft entry picks written to sample file.")
        return

    picks, inserted, unauthorized = collect_draft_entry_picks(token, draft_ids, conn=conn)
    if unauthorized:
